 ...
 */
export function runChangesetVersion(cwd: string, dryRun: boolean = false): VersionedPackage[] {
  if (dryRun) {
  ...
  try {
    ...
    throw new Error(`Failed to run changeset version: ${String(error)}`, { cause: error });
  }
...
function findPackageInfo(cwd: string, name: string): { path: string; version: string } | null {
